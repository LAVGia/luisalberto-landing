...
export default function Invitacion() {
  ...
  // Estado para la cuenta regresiva dinámica
  ...
  useEffect(() => {
    setIsMounted(true)
    // Fecha del evento: 18 de Octubre de 2026 a las 17:00 (5:00 PM)
    const targetDate = new Date("October 18, 2026 17:00:00").getTime()
    ...
    const interval = setInterval(() => {
      ...
      if (difference > 0) {
        setTimeLeft({
          ...
          horas: Math.floor((difference % (1000 * 60 * 60 * 24)) / (1000 * 
60 * 60)),
          minutos: Math.floor((difference % (1000 * 60 * 60)) / (1000 * 
60)),
...
transition-opacity duration-1000 ${isMounted ? 'opacity-100' : 
'opacity-0'}`}>
          {[
...
px-10 py-8 min-w-[140px] shadow-sm flex flex-col items-center"
            >
              <h3 className="text-3xl md:text-4xl font-semibold 
text-[#1a1a1a]">
                {String(item.value).padStart(2, '0')}
              </h3>
              <p className="uppercase tracking-[0.2em] text-[#9c8b6d] mt-2 
text-xs">
                {item.label}
              </p>
            </div>
...
text-sm">
            ...
            <h3 className="text-4xl font-light mb-8">Hacienda Santa 
Lucía</h3>
...
text-sm">
            ...
            <h3 className="text-4xl font-light mb-8">Jardines 
Bellavista</h3>
            <p className="text-[#555] text-lg leading-relaxed">
...
p-8 flex items-center justify-between shadow-sm"
              >
                <div className="flex items-center gap-6">
                  ...
                  <div>
                    <h3 className="text-2xl font-medium">{item[1]}</h3>
                    <p className="text-[#777] mt-2">{item[0]}</p>
                  </div>
                </div>
              </div>
            ))}
...
text-sm">
        ...
        <h2 className="text-5xl md:text-6xl font-light">Formal 
Elegante</h2>
...
leading-relaxed">
      ...
      <section className="py-28 px-8 md:px-20">
        <h2 className="text-5xl font-light text-center mb-20">Nuestra 
Historia</h2>
...
group-hover:translate-x-full transition duration-1000"></span>
          <span className="relative z-10 text-[#b38b2d]">✨</span>
